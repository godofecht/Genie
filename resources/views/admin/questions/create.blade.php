    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.questions.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('type') ? 'is-invalid' : '' }}" name="type" id="type" required>
                    ...
                    @foreach(App\Models\Question::TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('type', 'single_line') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('type'))
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('is_required') ? 'is-invalid' : '' }}" name="is_required" id="is_required" required>
                    ...
                    @foreach(App\Models\Question::IS_REQUIRED_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('is_required', 'required') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('minimum_answer_length') ? 'is-invalid' : '' }}" type="number" name="minimum_answer_length" id="minimum_answer_length" value="{{ old('minimum_answer_length', '0') }}" step="1" required>
