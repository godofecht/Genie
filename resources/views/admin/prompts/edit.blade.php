    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.prompts.update", [$prompt->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group" style="display: {{ \Laravel\Pennant\Feature::active('image-prompts') ? '' : 'none' }}">
                ...
                <select class="form-control {{ $errors->has('type') ? 'is-invalid' : '' }}" name="type" id="type" required>
                    @foreach(App\Models\Prompt::TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('type', $prompt->type) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('type'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}" type="text" name="title" id="title" value="{{ old('title', $prompt->title) }}" required>
                @if($errors->has('title'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" type="text" name="description" id="description" value="{{ old('description', $prompt->description) }}" required>
                @if($errors->has('description'))
                    <span class="text-danger">{{ $errors->first('description') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.prompt.fields.description_helper') }}</span>
            </div>
            <div class="form-group" style="display: {{ \Laravel\Pennant\Feature::active('dynamic-prompts') ? '' : 'none' }}">
                <label class="required" for="prompt">{{ trans('cruds.prompt.fields.prompt') }}</label>
                <textarea class="form-control ckeditor {{ $errors->has('prompt') ? 'is-invalid' : '' }}" name="prompt" id="prompt" required>{!! old('prompt', $prompt->prompt) !!}</textarea>
            ...
            <div class="form-group" style="display: {{ \Laravel\Pennant\Feature::active('dynamic-prompts') ? '' : 'none' }}">
                ...
                <select class="form-control select2 {{ $errors->has('engine') ? 'is-invalid' : '' }}" name="engine_id" id="engine_id">
                    @foreach($engines as $id => $entry)
                        <option value="{{ $id }}" {{ (old('engine_id') ? old('engine_id') : $prompt->engine->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group" style="display: {{ \Laravel\Pennant\Feature::active('dynamic-prompts') ? '' : 'none' }}">
                ...
                <input class="form-control {{ $errors->has('max_tokens') ? 'is-invalid' : '' }}" type="number" name="max_tokens" id="max_tokens" value="{{ old('max_tokens', $prompt->max_tokens) }}" step="1">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('category') ? 'is-invalid' : '' }}" name="category_id" id="category_id" required>
                    @foreach($categories as $id => $entry)
                        <option value="{{ $id }}" {{ (old('category_id') ? old('category_id') : $prompt->category->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group" style="display: {{ \Laravel\Pennant\Feature::active('dynamic-prompts') ? '' : 'none' }}">
                ...
                <select class="form-control select2 {{ $errors->has('tone') ? 'is-invalid' : '' }}" name="tone_id" id="tone_id">
                    @foreach($tones as $id => $entry)
                        <option value="{{ $id }}" {{ (old('tone_id') ? old('tone_id') : $prompt->tone->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('tone'))
                                    ...
                                    <div class="col-xs-10">
                                        <select class="form-control question-select" name="questions[]">
                                            @foreach($questions as $id => $q)
                                                <option
                                                    value="{{ $id }}"{{ $id === $question->id ? ' selected' : '' }}>{{ $q }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="col-xs-2 ml-3">
